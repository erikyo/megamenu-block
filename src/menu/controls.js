...
function Controls( args ) {
	...
	function setAlignment( alignment ) {
		return () => {
			const itemsJustification =
				attributes.itemsJustification === alignment
					? undefined
					: alignment;
			setAttributes( {
				itemsJustification,
	...
	useEffect( () => {
		updateChildBlocksAttributes( {
			...
			customFontSize: menuItemFontSize.slug
				? undefined
	...
	useEffect( () => {
		updateChildBlocksAttributes( {
			...
			customTextColor: menuItemColor.slug
				? undefined
				: menuItemColor.color,
		} );
	}, [ menuItemColor.color ] );
	...
	const setMenuItemFontSize = ( value ) => {
		...
		setAttributes( {
			...
			customMenuItemFontSize: fontSizeSlug
				? undefined
				: value?.toString(),
		} );
	};
		...
		<>
				...
				<Toolbar
					icon={
						attributes.itemsJustification
							? `editor-align${ attributes.itemsJustification }`
							: 'editor-alignleft'
					}
          ...
          />
				...
				>
					<RangeControl
						label={ __(
							'Maximum width of top-level menu in pixels'
						) }
						value={ attributes.menuMaxWidth }
						onChange={ ( value ) =>
							setAttributes( { menuMaxWidth: value } )
						}
						min={ 0 }
						max={ 2000 }
					/>
					...
					<RangeControl
						label={ __(
							'Maximum width of dropdown content in pixels'
						) }
						value={ attributes.dropdownContentMaxWidth }
						onChange={ ( dropdownContentMaxWidth ) =>
							setAttributes( { dropdownContentMaxWidth } )
						}
						min={ 0 }
						max={ 2000 }
					/>
					...
					<BaseControl label={ __( 'Toggle button alignment' ) }>
						<ButtonGroup>
							<Button
								...
								isPrimary={
									'left' === attributes.toggleButtonAlignment
								}
							/>
							<Button
								icon="editor-aligncenter"
								...
								isPrimary={
									'center' ===
									attributes.toggleButtonAlignment
								}
							/>
							<Button
								icon="editor-alignright"
								...
								isPrimary={
									'right' === attributes.toggleButtonAlignment
								}
							/>
						</ButtonGroup>
					</BaseControl>
